    ...
    <PageHeader :title="category.category.name" :subtitle="description">
      <template #breadcrumb>
        ...
        <nav
          ...
          class="flex flex-wrap items-center justify-center gap-1.5 text-sm text-ink-on-deep/75"
        >
          <NuxtLink :to="localePath('/')" class="transition-colors hover:text-ink-on-deep">
            {{ t("nav.home") }}
          </NuxtLink>
          <template v-for="ancestor in category.ancestors" :key="ancestor.id">
            ...
            <NuxtLink
              ...
              class="transition-colors hover:text-ink-on-deep"
            >
              {{ ancestor.name }}
            </NuxtLink>
          </template>
          <span aria-hidden="true">/</span>
          <span class="text-ink-on-deep">{{ category.category.name }}</span>
        </nav>
      </template>
