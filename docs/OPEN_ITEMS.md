...
| S-8 | **UI rework, stages B–D** | — | **Stage A done** — CHG-0063 (tokens, type, shape), plus the light/dark switch (CHG-0065), the collapsible rail and the app frame (CHG-0066, CHG-0067). Still owed: **B** icons, logo, favicon — 8 files carry hand-written SVG and neither app has a `public/` directory; **C** the primitive set — 6 primitives against 28 page files, which is the one that compounds as Phase 3 adds screens; **D** page-level layout. **The staged plan with per-task detail is [UI_REWORK.md](UI_REWORK.md)**; individual visual defects are in [UI_DEFECTS.md](UI_DEFECTS.md). |
| ~~S-9~~ | ~~**Wire a dark-mode toggle**~~ | — | **Done — CHG-0065.** `DbThemeToggle` in the site header and both app shells, persisted in a `databro_theme` cookie shared across apps like `databro_locale`. Applied by a pre-paint head script rather than SSR markup: `site` is ISR-cached, so a theme rendered into the HTML would be one visitor's choice served to everyone behind the cache. Following `prefers-color-scheme` stays unwired on purpose (ADR-0020 §6) — the switch is the opt-in. |
