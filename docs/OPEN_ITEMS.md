...
| S-8 | **UI rework, stages B–D** | — | Stage A (tokens, type, shape, primitives) **done — CHG-0063**. The **rail and the shared 1760px frame** landed in CHG-0066, along with the card surfaces. Still owed: **B** a real icon system, a logo and a favicon — the rail's nine glyphs are inline SVG and there is still no `public/` directory in either app; **C** the rest of the primitive set (Select, Modal, Tabs, Table, Avatar, EmptyState, Skeleton, Toast, Pagination); **D** page-level layout against the references. Individual visual defects are logged in [UI_DEFECTS.md](UI_DEFECTS.md). **C is the one that compounds** — the primitives are still thin against 56 page files, and Phase 3 adds dozens more screens. |
