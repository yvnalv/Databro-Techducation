    ...
    IUserDirectory userDirectory)
{
    ...
    // Content owns only an author id; display names come from Identity through the shared
    // IUserDirectory contract (ADR-0008). Resolved in one batched call per request so list
    // endpoints never fan out into N lookups.
    private Task<IReadOnlyDictionary<Guid, UserSummary>> ResolveAuthorsAsync(
        IEnumerable<Article> articles, CancellationToken ct) =>
        userDirectory.GetUsersAsync(articles.Select(a => a.AuthorId).Distinct().ToArray(), ct);

    private Task<IReadOnlyDictionary<Guid, UserSummary>> ResolveAuthorAsync(
        Article article, CancellationToken ct) =>
        userDirectory.GetUsersAsync([article.AuthorId], ct);
    ...
    public async Task<Result<ArticleDto>> CreateDraftAsync(CreateArticleRequest request, CancellationToken ct = default)
    {
        ...
        return Result.Success(article.ToDraftDto(await ResolveAuthorAsync(article, ct)));
    }
    ...
    public async Task<Result<ArticleDto>> PublishAsync(Guid id, CancellationToken ct = default)
    {
        ...
        await repository.SaveChangesAsync(ct);
        return Result.Success(article.ToDto(await ResolveAuthorAsync(article, ct)));
    }
    ...
    public async Task<ArticleDto?> GetPublishedBySlugAsync(string slug, CancellationToken ct = default)
    {
        ...
        return article is null ? null : article.ToDto(await ResolveAuthorAsync(article, ct));
    }

    public async Task<ArticleDto?> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        var article = await repository.GetByIdAsync(id, ct);
        return article is null ? null : article.ToDraftDto(await ResolveAuthorAsync(article, ct));
    }
    ...
    public async Task<IReadOnlyList<ArticleSummaryDto>> ListPublishedAsync(int limit = 20, CancellationToken ct = default)
    {
        ...
        var authors = await ResolveAuthorsAsync(articles, ct);
        return articles.Select(a => a.ToSummaryDto(authors)).ToList();
    }
